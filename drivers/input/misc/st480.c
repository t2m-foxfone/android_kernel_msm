...
static struct sensors_classdev sensors_compass_cdev = {
	.name = "ST480 ORT",
	.vendor = "Senodia",
	.version = 1,
...
struct st480_data {
	...
	struct sensors_classdev compass_cdev;
};
...
int st480_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	st480->compass_cdev = sensors_compass_cdev;
	//st480->compass_cdev.sensors_enable;
	err = sensors_classdev_register(&client->dev, &st480->compass_cdev);
